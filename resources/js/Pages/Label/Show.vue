...
<template>
    <TaskLayout>
        <div class="w-96 mx-auto pt-5">
            ...
            <h1 class="text-lg  text-center">Метка </h1>
                <Link :href="route('labels.create')" class="px-3 py-1 hover:bg-white hover:text-sky-400 border border-sky-400 rounded-full bg-sky-400 text-center text-white">Добавить метку</Link>
            </div>
            <div class="mt-5">
                <div class="mt-2 pt-4 border-t border-gray-300" >
                    <div class="text-center text-2xl mb-5"> {{ label.title }}</div>
                    <div><span class="text-sm  mr-2 text-slate-500">ID:</span><span
                        class="text-sm text-gray-700">{{ label.id }}</span></div>
                    <div><span class="text-sm  mr-2 text-slate-500">Добавлено:</span><span
                        class="text-sm text-gray-700">{{ label.date }}</span></div>
                    <div class="mb-5"><span class="text-sm  mr-2 text-slate-500">Последнее редактирование:</span><span
                        class="text-sm text-gray-700">{{ label.updated_date }}</span></div>
                    <div class="text-sm text-right">
                        <Link class="hover:text-sky-300 text-sky-500" :href="route('labels.edit', label.id)">
                            Редактировать
                        </Link>
                    </div>
